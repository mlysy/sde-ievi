...
class RNN(eqx.Module):
    ...
    def __call__(
        ...
        par_ind = [
            self.n_state * self.n_res,
            self.n_state * self.n_state * self.n_res,
            self.n_state * (self.n_state + 1) * self.n_res // 2,
        ]
        ...
        def f(carry, fargs):
            ...
            wgt_state = out[par_indices[0] : par_indices[1]].reshape(
                self.n_res, self.n_state, self.n_state
            )
            ...
            chol_state = (
                chol_state.at[diag_ind].set(jax.nn.softplus(chol_state[diag_ind])).T
            )
            ...
            def kalman_scan(carry, kwargs):
                ...
                # output
                carry = {"state_filt": (mean_state_pred, var_state_pred)}
                stack = {
            ...
            kalman_init = {"state_filt": (mean_state_filt, var_state_filt)}
        ...
        scan_init = {"hidden": hidden, "state_filt": (mean_state_init, var_state_init)}
        fargs = {"data_seq": data_seq, "x_meas": y_meas}

        final, scan_out = jax.lax.scan(f, scan_init, fargs)
        scan_out["state_filt"] = (
            jnp.concatenate(
                [
                    mean_state_init[None],
                    scan_out["state_filt"][0].reshape(-1, self.n_state),
                ]
            ),
            jnp.concatenate(
                [
                    var_state_init[None],
                    scan_out["state_filt"][1].reshape(-1, self.n_state, self.n_state),
                ]
            ),
        )
        scan_out["state_pred"] = (
        ...
        scan_out["wgt_state"] = scan_out["wgt_state"].reshape(
            -1, self.n_state, self.n_state
        )
        return scan_out
...
# can use this extra neural network to make the smoothing distribution conditional
# class NN(eqx.Module):
#     hidden_size: int
#     out_size: int
#     layers: list
...
#             jax.nn.relu,
#             eqx.nn.Linear(
#                 self.hidden_size, self.hidden_size, key=subkey[1], use_bias=True
#             ),
#             jax.nn.relu,
#             eqx.nn.Linear(
#                 self.hidden_size, self.hidden_size, key=subkey[2], use_bias=True
#             ),
#             jax.nn.relu,
#         ]
#         self.linear = eqx.nn.Linear(
#             self.hidden_size, self.out_size, key=subkey[4], use_bias=True
#         )
...
class KalmanModel:
    ...
    $$
    by fitting the Kalman filtering parameters. The main difference between this model and the standard `model_svi`
    is that the model relies on the Kalman update to figure out the filtering estimates 
    (i.e., it relies on using the fact that measurements are assumed to be Gaussian.) 
    ...
    n_theta : int
        Number of parameters for inference.
    n_res : int
        Resolution number.
    wgt_meas : jax.Array
    ...
    def simulate_theta(self, key, params, y_meas):
        ...
        theta_entpy = -jnp.sum(
            jax.scipy.stats.multivariate_normal.logpdf(
                theta, theta_mu, theta_chol.dot(theta_chol.T)
            )
        )
    ...
    def simulate(self, key, params, y_meas, x_init, theta, obs_times):
        ...
        y_meas = jax.vmap(lambda x, y: y - self._wgt_meas.dot(x))(
            x_init[:: self._n_res], y_meas
        )
        ...
        def scan_fun(carry, fwd_kwargs):
            ...
            x_neglogpdf -= jax.scipy.stats.multivariate_normal.logpdf(
                x_state_curr, mean_state_curr, var_state_curr
            )
            carry = {"x_state_next": x_state_curr, "x_neglogpdf": x_neglogpdf}
            return carry, carry

        # time N
        mean_state_N = mean_state_filt[-1]
        var_state_N = var_state_filt[-1]
        random_normals = jax.random.normal(
            key, shape=(len(mean_state_filt), self._n_state)
        )
        chol_factor = jnp.linalg.cholesky(var_state_N)
        x_N = mean_state_N + chol_factor.dot(random_normals[-1])
        x_neglogpdf = -jax.scipy.stats.multivariate_normal.logpdf(
            x_N, mean_state_N, var_state_N
        )
        scan_init = {"x_state_next": x_N, "x_neglogpdf": x_neglogpdf}
        # scan arguments
        scan_kwargs = {
        ...
        last_out, stack_out = jax.lax.scan(
            scan_fun, scan_init, scan_kwargs, reverse=True
        )
        x_state_smooth = (
            jnp.concatenate([stack_out["x_state_next"], x_N[None]]) + x_init
        )
        # calculate -E[log q(x, theta|phi_full)]
        # break down to - E[log q(theta)] - E[log q(x|theta)]
