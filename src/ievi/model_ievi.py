...
class RNN(eqx.Module):
    ...
    def __init__(self, key, n_state, n_meas):
        ...
        self.linear = eqx.nn.Linear(
            self.hidden_size, self.hidden_size // 2, key=subkey[2]
        )
...
class SmoothModel:
    ...
    def _par_parse(self, params, obs_theta):
        ...
        par_indices = [
            self._n_state,
            2 * self._n_state,
            self._n_state * (2 + self._n_state),
            self._n_state * (3 * self._n_state + 5) // 2,
            self._n_state * (2 * self._n_state + 3),
        ]
        mean_state_filt = full_par[:, : par_indices[0]]
        mean_state = self._dt * full_par[:, par_indices[0] : par_indices[1]]
        wgt_state = full_par[:, par_indices[1] : par_indices[2]].reshape(
            self._n_sde, self._n_state, self._n_state
        )
        ...
        chol_state_filt = (
            chol_state_filt.at[upper_ind].set(
                full_par[:, par_indices[2] : par_indices[3]].T
            )
            * 0.1
        )
        chol_state_filt = (
            chol_state_filt.at[diag_ind]
            .set(jax.nn.softplus(chol_state_filt[diag_ind]))
            .T
        )
        chol_state = jnp.zeros((self._n_state, self._n_state, self._n_sde))
        chol_state = chol_state.at[upper_ind].set(
            full_par[:, par_indices[3] : par_indices[4]].T
        )
        chol_state = (
    ...
    def simulate_theta(self, key, params):
        ...
        theta_entpy = 0.5 * n_theta * (1 + jnp.log(2 * jnp.pi)) + jnp.sum(
            jnp.log(jnp.diag(theta_chol))
        )
    ...
    def simulate(self, key, params, y_meas, x_init, theta):
        ...
        mean_state_filt, var_state_filt, mean_state_pred, var_state_pred, wgt_state = (
            self._par_parse(params, obs_theta)
        )
